...
function BanRiskBadge({ risk }: { risk: DriverStats["ban_risk"] }) {
  const styles: Record<string, string> = {
    none:   "text-gray-500 border-gray-800",
    ...
    <main className="max-w-3xl mx-auto px-4 py-10 space-y-8">
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className="border border-gray-800 rounded p-3 text-center">
          ...
          <div className="text-xs text-gray-600">All-time points</div>
        </div>
        <div className="border border-gray-800 rounded p-3 text-center">
          <div className="text-2xl font-mono font-bold">{stats.current_season_points}</div>
          <div className="text-xs text-gray-600">2025 points</div>
        </div>
      </div>
      ...
      <section className="space-y-3">
        ...
        <div className="flex flex-wrap gap-2">
          {PENALTY_CLASSES.map((cls) => {
            ...
            return (
              <div key={cls} className="border border-gray-800 rounded p-3 text-center min-w-[60px]">
                <div className="text-lg font-mono font-bold">{count}</div>
                <div className="text-xs text-gray-600 font-mono">{cls}</div>
              </div>
            );
          })}
        </div>
      ...
      <section className="space-y-2">
        ...
        <div className="space-y-1">
          {stats.incidents.map((inc) => (
            <div
              key={inc.incident_id}
              className="border border-gray-800 rounded p-3 space-y-1 hover:border-gray-700 transition-colors"
            >
              <div className="flex items-center justify-between gap-2">
                <span className="text-sm font-medium leading-snug line-clamp-1">{inc.title}</span>
                <div className="flex items-center gap-2 shrink-0">
                  <span className="text-xs text-gray-600">{inc.season}</span>
                  {inc.penalty_type && (
